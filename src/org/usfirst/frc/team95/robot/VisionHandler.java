...
public class VisionHandler {
	double x, y, aimX, aimY, distance, power = 1; // These represent the x and
													// y, in pixels, of the
	// center of the goal.
	...
	private final static String[] CLEAR_TMP_CMD = { "/bin/rm", "-rf", "/tmp/*" };

	private final static String[] GRIP_CMD = { "/usr/local/frc/JRE/bin/java", "-jar", "/home/lvuser/grip.jar",
			"/home/lvuser/project.grip" };

	private final static String[] KILL_REMOTE_GRIP = { "ssh", "pi@raspberrypi.local", "sudo", "killall", "java" };

	static VisionHandler instance; // This is the only instance of VisionHandler
									// that should be used

	NetworkTable GRIPTable; // This represents GRIPs published reports
	Comparator<Line> sort = new Comparator<Line>() { // This sorts Line objects
														// based on length
	...
	ITableListener updater = new ITableListener() { // This updates the x and y
													// whenever the table
													// updates.
	...
	public boolean horizontalP(double degrees) { // This determines if an angle
	...
	public void update(ITable table) { // This does the workhorsing of the
										// updates
		// System.out.println(table.getKeys());
		Line[] lineTable = getLines(table);
		// System.out.println(lineTable.length);

		ArrayList<Line> lines = new ArrayList<Line>(); // This would be a Set,
														// if I could get sets
														// working.
		...
		distance = 1 / Math.tan(tY)
				* (Constants.goalHeight - Math.sin(RobotMap.arm1.getPosition()) * Constants.cameraDistanceToPivot);

		// double V0y =
		// Math.sqrt(2*Constants.gravity*Math.sin(RobotMap.arm1.getPosition())*
	...
	public Line[] getLines(ITable lineReport) { // This marshals stuff from the
												// NetworkTable to
